...
Rollback never selects the immediately previous deployment. Before rollback, an operator writes a mode-`0600`, ignored LKG receipt containing the explicit Worker/version/deployment, source commit/tree, script ETag, full non-secret resource fingerprint, verifier, and verification time. The rollback command independently rereads that exact version and deployment and rejects missing or ambiguous targets, split traffic, wrong environment/source/resources, any dangerous flag that is true or missing, non-allowlist delivery, or a nonempty allowlist.

Use `pnpm cloudflare:run verify-lkg --environment <ENVIRONMENT> --lkg-receipt .cloudflare/lkg.<environment>.json` for the same read-only verification before opening a rollback window.

Production deploy also requires `--dlq-baseline .cloudflare/dlq.production.json`. That mode-`0600` operator receipt names the exact projection DLQ ID and accepted count/bytes/oldest-message metric with its observation time and verifier. The deploy gate rereads all six production queues twice, 10–60 seconds apart. Every active queue and the email DLQ must be exactly zero. A retained projection DLQ is allowed only when it is at or below a baseline accepted within six hours; any positive count/byte delta from baseline or between the two reads fails closed. Never reuse this exception for another queue, omit the exact Queue ID, or treat an old single observation as proof of safety.

Run the identical read-only gate before opening the release window with `pnpm cloudflare:run verify-queues --environment production --dlq-baseline .cloudflare/dlq.production.json --queue-observation-seconds 30`. A successful check writes an ignored mode-`0600` two-measurement receipt; production deploy repeats the gate rather than trusting that earlier result.

Any production deploy whose target has `writes=true` must also supply `--writes-window-start <UTC>` and `--writes-window-end <UTC>`. The provisioner reads the hourly reconciliation Cron from the target Wrangler config, limits the planned window to ten minutes, adds two minutes of safety on both sides, and rejects any overlap with the Cron minute (`:17` in the current config), including boundary, cross-hour, and timezone-normalization cases. There is no override. The writable deploy emits an ignored mode-`0600` receipt bound to the active version. Relock that exact version with `--writes-window-receipt <PATH>`; the provisioner activates the locked version first, then polls the durable `authority.full_scan.*` telemetry and all Queue metrics at the configured 10–60 second interval for at most four minutes. If a Cron may have fired, every expected scan must have a terminal success event, and two post-scan Queue reads must converge under the retained-DLQ gate before smoke or release acceptance. Missing, failed, or timed-out evidence fails closed. This guard exists because the RAL-111 incident window crossed `:17`, allowing an already-fired full scan and its invalidations to outlive relock.

The deterministic demo bootstrap is a separate operator-only release step. Run it only after the complete forward migration chain through `0026_event_contact_identity_bindings.sql`, Airtable schema v10, and the same immutable Worker SHA are active. It refuses a foreign or duplicate Airtable root, writes the two canonical roots through `AirtableCommandStore`, registers the exact base and source-record lineage with readiness unset, synchronizes through `BaseAuthority`, and only then applies and verifies the digest-bound 139-operation snapshot plus four private assets. It does not expose a public bootstrap screen or accept an arbitrary event.
...
After any reset that intentionally clears demo auth identities, keep email disabled and use the owner-only role-identity plan/provision contract documented in the operator guide. It binds exactly one runtime alias to each named organizer, reviewer, and speaker identity for the compiled demo tenant/event, emits no email, and must complete before the separately approved bounded send lane. Never copy the aliases into release evidence.
...
- Worker deployment rollback uses only an explicitly named, independently read, fully locked last-known-good version/deployment receipt; list position is never a selection signal.
...
- Run `pnpm cloudflare:status` to read the active version. Create the ignored mode-`0600` LKG receipt from the independently verified release record, then run `pnpm cloudflare:rollback:preview -- --lkg-receipt .cloudflare/lkg.preview.json`. The equivalent lower-level production command additionally requires `--confirm-production` and `CLOUDFLARE_PRODUCTION_CONFIRM=production`.
