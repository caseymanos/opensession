...
- Organizer submission commands use the same authority with a D1 receipt that freezes stable child operations. Lifecycle writes carry the projected source-version precondition. Adding a note version-touches the submission before creating the linked `Submission Notes` record, so concurrent commands serialize and fail stale rather than bypassing authority through D1. The organizer workspace remains unavailable for writes until Airtable schema v5 and D1 migration `0016_organizer_submissions.sql` are both ready.
