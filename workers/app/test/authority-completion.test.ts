...
let roomOperation: CompiledDemoSeed["operations"][number];
...
beforeAll(async () => {
  ...
  const compiledRoom = plan.operations.find(({ table }) => table === "rooms");
  if (!compiledRoom) throw new Error("Demo fixture is missing a room.");
...
describe.sequential("RAL-34 completed authority data plane", () => {
  it("reconciles every table and fails closed on roster and source changes", async () => {
    expect(
  ...
  it("replaces one exact demo snapshot with durable asset recovery and scoped deletion", async () => {
  ...
  it("revokes incomplete authority and drains durable cache invalidations", async () => {
