...
function RubricEditor({
  ...
  const submittedVersionLabel = Array.from(new Set(submittedSnapshots))
    .sort()
  ...
  return (
    <section aria-labelledby="rubric-title" className="review-ops-panel">
      ...
      <div className="review-ops-snapshot-note">
        ...
        <div>
          <strong>
            Submitted reviews stay on rubric {submittedVersionLabel}.
          </strong>
          <p>
            Publishing a new version affects unstarted assignments only.
            {` ${submittedSnapshots.length} submitted review snapshot remains unchanged.`}
          </p>
        </div>
      </div>
      ...
      <Dialog
        description={`Unstarted assignments will move to v${activeVersion + 1}. Submitted review snapshots remain on ${submittedVersionLabel}.`}
      ...
      >
        <div className="review-ops-publish-dialog">
          ...
          <div>
            ...
            <span>Existing submitted scores retain their original rubric</span>
          </div>
...
function ReviewerGroups({
  ...
  onChange: (groups: ReviewerGroupView[], action: "added" | "removed") => void;
}) {
  ...
  const normalizedReviewer = newReviewer.trim();
    ...
    onChange(
      groups.map((group) =>
        group.id === groupId
          ? {
              ...group,
              members: group.members.filter((item) => item !== member),
            }
          : group,
      ),
      "removed",
    ...
    onChange(
      groups.map((group) =>
        group.id === addGroup.id
          ? { ...group, members: [...group.members, normalizedReviewer] }
          : group,
      ),
      "added",
    );
  ...
  return (
    <section aria-labelledby="groups-title" className="review-ops-panel">
      <div className="review-ops-panel-heading">
        ...
        <StatusPill tone="success">4 of 4 routes mapped</StatusPill>
      </div>
      <div className="review-ops-route-proof">
        <Route aria-hidden="true" size={18} />
        <span>
          <strong>Track D proof:</strong> Product submissions route to Product
          reviewers; Casey Brooks sees AES-1120 and rubric v2 only.
        </span>
      </div>
...
function Assignments({
  ...
  function addAssignment() {
    const source = assignments.find(
      (assignment) => assignment.proposalReference === selectedProposal,
    );
    if (!source) return;
    const id = `${source.proposalReference}-${selectedReviewer}`
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-");
          ...
          assignment.id === id
            ? {
                ...
                audit: [
                  ...
                  {
                    actor: "Casey Manos",
                    detail: `Restored ${selectedReviewer}'s assignment and proposal access`,
                    time: "Just now",
                  },
                ],
        ...
        [
          ...assignments,
          {
            ...
            audit: [
              {
                actor: "Casey Manos",
                detail: `Assigned ${selectedReviewer}`,
                time: "Just now",
              },
            ],
            id,
        ...
        <div className="review-ops-assign-dialog">
          ...
          <SelectField
            ...
            options={[
              ...
              "Noah Williams",
              "Inez Park",
            ].map((reviewer) => ({ label: reviewer, value: reviewer }))}
...
export function ReviewOperations() {
  ...
  function announce(title: string, message: string) {
    ...
    setToasts((current) => [
      ...current,
      { id: `${Date.now()}`, message, title, tone: "success" },
    ]);
  }
      ...
      {tab === "rubric" ? (
        <RubricEditor
          ...
          onPublish={() => {
              ...
              setAssignments((current) =>
                current.map((assignment) =>
                  assignment.status === "pending"
                    ? {
                        ...assignment,
                        rubricSnapshot: cloneRubricSnapshot(publishedSnapshot),
            ...
            announce(
              "Rubric published",
              "Unstarted assignments now use the new rubric; submitted snapshots did not change.",
            );
          }}
          submittedSnapshots={assignments
            .filter((assignment) => assignment.status === "submitted")
            .map((assignment) => assignment.rubricVersion)}
        />
      ) : tab === "groups" ? (
        <ReviewerGroups
          groups={groups}
